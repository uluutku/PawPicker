    ...
    return (
        <Box sx={styles.uploadPage}>
            <Paper sx={styles.uploadContainer}>
                <Typography variant="h6" sx={{ fontFamily: 'body1.fontFamily' }}>Throw your pictures into the wizard&apos;s cauldron</Typography>
                ...
                <Grid container spacing={2} sx={{ mt: 2 }}>
                    {selectedFiles.map((file, index) => (
                        <Grid item xs={6} sm={4} md={3} key={index} sx={styles.gridItem}>
                            ...
                            <IconButton onClick={() => handleDelete(file.url)} sx={styles.deleteIcon}>
                                <DeleteIcon />
                            </IconButton>
                        </Grid>
                    ))}
                </Grid>
                <Button variant="contained" color="primary" onClick={handleUpload} disabled={!selectedFiles.length} sx={styles.button}>
